...
export default function JarCard({ jar }: JarCardProps) {
  ...
  return (
    <Link
      to={`/products/${jarSlug(jar)}`}
      className="block w-36 shrink-0 snap-start [&>div]:transition-colors hover:[&>div]:border-hyperlayer-grey/20 hover:[&>div]:shadow-sm"
    >
      <div className="flex h-36 flex-col rounded-xl border border-hyperlayer-grey/10 bg-white p-3">
